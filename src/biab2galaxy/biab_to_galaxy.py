# TODO
# gérer les collections en outputs
    # en biab les collections sont des vecteurs ou des listes de chemins de fichiers et
# avoir des noms d'attributs invariables pour les types spéciaux (pour faciliter la gestion des outputs de types spéciaux)

# gérer les dépendances non listées dans le xml et installées par le script

# s'il n'y a que les types spéciaux qui ont besoin des .loc et que les types spéciaux sont gérés par biab form, alors seul biab form a besoin des .loc
# est-ce que les .loc peuvent faire partie des fichier de l'outil ?

# modifier biab_form pour que les fichiers .loc soient dans le repertoire de l'outil lui même ou voir si ça peut pas être intégéré directement dans le xml

# TODO ne plus afficher plusieurs exemples comme une liste

# pour régler les dépendences il faut :
    # créer un fichier de dépendances pour les packages installés dans le script
    # ajouter les dépendances crées aux requirements
    # obtenir une recette avec conda avec les versions
    # dans cette recherche, trouver les packages des requirements, garder en mémoire leur version
    # créer un fichier de recette conda à partir de ça
# Est-ce que c'est nécéssaire si Galaxy enregistre la recette stable ?

# et si j'intsallais le package github directement dans le dossier de l'outil comme ?
    # de toute manière un biocontainer n'est pas la solution, les développeurs devront s'occuper eux même de gérer leur dépendances.
# ou alors éxécuter les commande de tool dependencies au moment de la génération de l'outil en mode instance.
    # il faudrait alors changer l'endroit ou installer les librairies parce que les repertoire des dépendances de l'outil est créé à la volée.
    
# si le chargement de librairie après leur installation ar le script ne fonctionne pas c'est que peut être le script n'a pas les droits en écriture des dépendances (une question de sécurité)
# il faudrait changer le repertoire d'installation pour choisir un qui ait les droits en écriture et lecture (get_cwd()) puis charger le package à ce repertoire.
    # dans la commande d'install ajouter le paramètre lib="./nom_du_package" et dans librairie ajouter le paramètre lib.loc="./nom_du_pakage"
# pourquoi pas installer les packages en dehors de l'exécution lors de la conversion dans le repertoire de l'outil ?

# prendre la partie du yaml qui définie les requirements, ça correspond extacrement à un fichier environment yaml qu'utilise Conda
# conda create --file environment.yml

# l'image docker sur laquelle se baser https://hub.docker.com/r/bgruening/galaxy-stable/

# gérer les exécutions de script dans les scripts
